...
export class MessagingComponent implements OnInit {
    ...
    sendMessage() {
        ...
        if ((message.forwarded.length === 0 && message.images.length === 0 && message.documents.length === 0) && message.text.trim().length === 0) {
            return;
        }
    ...
    loadMoreMessages() {
        if (this.messages.length % MESSAGES_AMOUNT !== 0) {
            return;
        }
    ...
    loadMorePreviews() {
        if (this.previews.length % PREVIEWS_AMOUNT !== 0) {
            return;
        }
    ...
    sendMessageTextFocus() {
        setTimeout(() => {
            if (this.sendMessageText) {
                this.sendMessageText.nativeElement.focus();
            }
        }, 0);
    }
    ...
    addToChat(user: User) {
        if (this.currentPreview.type.toString() !== PreviewType[PreviewType.CHAT]) {
            return;
        }
